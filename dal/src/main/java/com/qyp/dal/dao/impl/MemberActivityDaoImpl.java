...
public class MemberActivityDaoImpl implements MemberActivityDao {
	...
	public List<MemberActivity> getMemberListByActivityId(String activityId) {
		...
		example.createCriteria().andActivityIdEqualTo(
				Integer.valueOf(activityId));
		List<MemberActivity> memberActivitys = memberActivityMapper
	...
	/**
	 * 鎸夌収activityId 鍜宮emberId 璺熸柊瀹℃壒鐘舵��
	 */
	@Override
	public boolean updateMemberStatus(MemberActivity memberActivity) {
		...
		example.createCriteria()
				.andActivityIdEqualTo(
						Integer.valueOf(memberActivity.getActivityId()))
				.andMemberIdEqualTo(memberActivity.getMemberId());
		int result = memberActivityMapper.updateByExample(memberActivity,
				example);
		if (result == 1) {
			return true;
		}
	...
	@Override
	public boolean isExist(String memberId, Integer activityId) {
		MemberActivityExample example = new MemberActivityExample();
		example.createCriteria().andActivityIdEqualTo(activityId)
				.andMemberIdEqualTo(memberId);
		int result = memberActivityMapper.countByExample(example);
		if (result > 0) {
			return true;
		}
		return false;
